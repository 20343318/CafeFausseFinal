...
$CafeClusterRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $env:TEMP 'CafeFausse-api06-tests')
)
$CafeExpectedClusterRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $env:TEMP 'CafeFausse-api06-tests')
)
$CafeDataDir = Join-Path $CafeClusterRoot 'data'
$CafeMarker = Join-Path $CafeClusterRoot '.cafe-fausse-api06-test-cluster'
...
$CafeMarkerText = "CafeFausse API-06 disposable test cluster|port=$CafePort|database=$CafeDatabase"
