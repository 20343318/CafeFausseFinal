...
from .db.reservation_gateway import ReservationGateway
...
def _production_dependencies(settings: Settings, safe_logger) -> Dependencies:
    ...
    try:
        ...
        reservation_service = ReservationService(
            ReservationGateway(pool, acquire_timeout_ms=settings.pool_acquire_timeout_ms),
            deadline_ms=settings.mutation_deadline_ms,
            retry_policy=retry_policy,
            monotonic=time.monotonic,
            sleeper=time.sleep,
            uniform=uniform,
            retry_observer=lambda attempt: safe_logger.event(
                "retry", operation="OP-05", attempt=attempt, retry_class="mutation_transient"
            ),
            cleanup_failure_observer=lambda: safe_logger.event(
                "unexpected_error", severity="WARNING", operation="OP-05", retry_class="mutation_cleanup_failure"
            ),
        )
        return Dependencies(
            settings=settings,
    ...
    try:
        ...
        def finish_request(response):
            ...
            if route == "/api/v1/health/liveness":
                ...
                operation = "OP-02"
            elif route == "/api/v1/reservations":
                operation = "OP-05"
