...
11. `database/POSTGRESQL_CONTRACT_FOR_FLASK.md`, approved baseline version 1.0 and reconciled version 1.1;
...
| PostgreSQL interaction | First execute exactly `cafe_fausse.book_reservation(text,text,text,text,text,timestamp without time zone,smallint,integer,text)` at `READ COMMITTED`. Stable outcomes: `booked`, `booked_phone_notice`, `exact_retry`, `same_customer_overlap`, `customer_identity_mismatch`, `middle_initial_conflict`, `unavailable`, `invalid_request`, `invalid_database_configuration`. Frozen details are `requires_read_committed`, `invalid_normalized_input`, `configuration_row_count`, `operating_hours_population`, `restaurant_table_population`, `invalid_timezone`, `nonexistent_local_start`, `ambiguous_local_start`, `utc_offset_mismatch`, `date_outside_booking_window`, `insufficient_same_day_lead`, `start_before_opening`, `misaligned_start`, `end_after_closing`, `duration_or_party_size_out_of_range`, `no_capacity_sufficient_combination`, and `time_boundary_crossed_during_booking`. For `duration_or_party_size_out_of_range`, `invalid_request` identifies caller-controlled party size and `invalid_database_configuration` identifies server-controlled duration. After `booked`, `booked_phone_notice`, or `exact_retry`, use the existing migration-004 authorized reads to project only stored `customers.first_name`, `customers.middle_initial`, and `customers.last_name` by canonical email plus current `reservation_configuration.restaurant_timezone`. These database strings and columns remain internal traceability facts, except for the approved confirmation values derived from them. |
| Transaction character | Each booking attempt is one controlled atomic transaction; the routine call is the only statement in its explicit caller transaction or runs in autocommit. Commit a returned result; after exception roll back before any retry. Only after a successful booking or exact-retry result, perform the separate minimal read-only confirmation lookup for stored name components and current restaurant timezone; it neither changes nor extends the booking transaction. |
| Successful result | Confirmation reference; customer display name composed deterministically from the stored first name, optional middle initial, and last name obtained by the post-success canonical-email read; restaurant-local start/end serialized from the committed instants with the current post-commit restaurant IANA timezone; canonical start/end; party size; all sorted assigned table numbers; current newsletter state; optional differing-phone notice; and fixed SRS restaurant address/phone. The stored name is used for both a new booking and an exact retry, so a resubmission with different accepted letter casing reconstructs the same authoritative display spelling rather than echoing request casing. Do not claim delivery. |
...
| Privacy/minimization/logging | Confirmation email never reaches PostgreSQL. The post-success confirmation read projects only stored first name, optional middle initial, last name, and current restaurant timezone. The timezone is used only for confirmation local-time serialization. Return no customer ID, email, phone, profile data, other configuration, fingerprint, free/candidate tables, capacity internals, unrelated customer facts, or SQL diagnostics. Logs minimize/redact all PII and never record confirmation email or secrets. |
...
| Stable booking confirmation | OP-05 | Distinct confirmation view/retry recovery | Reference; stored first/optional middle/last name and current restaurant timezone from the post-success confirmation read; committed start/end serialized canonically and in that IANA timezone; party; all tables; newsletter; phone notice; fixed contact facts. Stored spelling, not resubmitted casing, is authoritative on exact retry. |
...
| UT-05 OP-05 booking | Single/multi-table; differing phone success/notice/no overwrite; new booking confirmation name comes from stored first/middle/last; exact retry with different accepted request casing returns the same stored display spelling, same reservation confirmation/current newsletter, and no mutation; post-success confirmation lookup projects no customer ID/email/phone/profile data or unrelated configuration; committed start/end use current restaurant IANA timezone, including offset transitions; invalid caller party size remains `invalid_request`; invalid server duration is `invalid_database_configuration`; same-customer overlap; different-customer overlap with capacity; back-to-back; stale availability -> unavailable; invalid local time/offset/window/lead/alignment/open/close; identity/middle conflicts; subscribe/unsubscribe/no-change; transient retry, exhaustion, timeout/loss before/after commit; unexpected failure no partial state; complete confirmation/no delivery claim | Deterministic routine classification, authoritative confirmation reconstruction, and safe recovery mapping | API-08/API-09 |
...
| IT-14 | Empty target interval with eligible single/multi combinations | OP-05; `book_reservation`, then stored-name and current-timezone confirmation projection | Booked/phone notice with stored display name and IANA-derived local interval | One customer, reservation, complete winning assignments, applicable newsletter | Existing customer protected fields; confirmation read exposes no ID/email/phone/profile or unrelated configuration facts | API-08 |
| IT-15 | Exact first result deliberately ignored/lost; repeat uses accepted casing variants of the same first/last name | Repeat OP-05; same routine, then the same minimal stored-name/current-timezone confirmation read | Exact-retry success with the original stored display spelling and current-timezone local interval | Exactly original rows; current newsletter and confirmation read | Contact/name/newsletter action not replayed; no non-name customer or unrelated configuration facts exposed | API-08 |
...
| IT-21 | Connection loss before/during/after commit, with an accepted casing variant on ordinary repeat | OP-05 then ordinary repeat and post-success minimal confirmation read | New/unknown then exact retry or fresh current result; success uses stored display spelling and current timezone | Zero or one logical reservation, never duplicate | Name/contact/newsletter nonreplay after exact commit; no customer ID/email/phone/profile or unrelated configuration exposure | API-08/API-09 |
...
The contract and implementation are compatible with every mandatory operation after the approved API08-RC-01/02 reconciliation recorded in this version. RC-01 revises only the booking outcome classification for invalid server-controlled duration; RC-02 authorizes only the minimum post-commit timezone projection needed for confirmation serialization.

## 25. Unresolved issues

No API-01 contract gap or privilege gap remains after the approved API08-RC-01/02 reconciliation. The subsequent API-08 correction must implement the reconciled classification and confirmation-timezone path.
...
| OP-05 confirmation reconstruction has deterministic authorized sources | Complete: post-success read projects stored first/middle/last plus current restaurant timezone only |
