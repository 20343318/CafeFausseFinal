...
The deterministic Ada fixture matches only case-insensitive full first/last identity for `ada.rivera@example.com`; omitted middle initial is permitted, `M` matches, punctuated `M.` is rejected, and another supplied initial returns `middle_initial_conflict`. OP-04 existing-customer true/false always returns `result:"set"`; unknown true returns `set/true`; unknown false returns `no_customer_no_change/false`. OP-05 uses stored fixture spelling and server fixture interval/preference facts: Ada `no_change` preserves subscribed, while subscribe/unsubscribe return their authoritative final Boolean.
...
Client first/last-name validation counts the normalized 100-character limit in Unicode code points, not UTF-16 code units, so those controls omit native `maxLength` where it would reject supplementary-plane letters. The optional middle-initial control exposes native `maxLength=1`; when supplied it accepts exactly one Unicode alphabetic character without a period, preserves valid input for submission, and reports `Enter one letter.` for invalid forms. The frozen email profile permits a valid single-label domain such as `ada@localhost`; invalid dot-atom, empty/invalid domain labels, edge hyphens, whitespace, comments, quoted locals, and domain literals remain rejected. The focused validation suite is the deterministic evidence for these boundaries.
